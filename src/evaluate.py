...
def _safe_load(model, ckpt_path, device):
    """Filter checkpoint weights by name and shape to drop 1000-class heads."""
    ...
    dropped = sorted(set(state.keys()) - set(ckpt.keys()))
    print(f"[safe_load] missing={len(missing)} unexpected={len(unexpected)} dropped={len(dropped)}")


@torch.no_grad()
def evaluate_once(checkpoint, data_dir, model_name, num_classes, image_size):
    device = _device()
    ...
    if args.save_csv:
        ...
        with open(args.save_csv, "w", newline="") as f:
            ...
            for imgs, labels in dl:
                logits = model(imgs.to(device))
                p = torch.softmax(logits, dim=1).cpu().numpy()
                for t, row in zip(labels.numpy().astype(int).tolist(), p.tolist()):
